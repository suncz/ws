...
class WSServer
{
    ...
    public function run()
    {
        $this->_server =$server = new swoole_websocket_server($this->_host, $this->_port, $this->_mode, $this->_socketType);
        $config=$this->_config;
        $this->_server->set($this->_config['setting']);
        $server->on('Start', function ($server) use ($config) {

            print_r('server start',Logger::LEVEL_INFO);
            swoole_set_process_name('php '.$config['serverName'].' master');
        });

        $server->on('ManagerStart', function ($server) use ($config) {
            print_r('ManagerStart',Logger::LEVEL_INFO);
            swoole_set_process_name('php '.$config['serverName'].' manager');
        });

        $server->on('ManagerStop', function ($server) {
            print_r('ManagerStop',Logger::LEVEL_INFO);
        });

        $server->on('WorkerStart', function ($server, $workerId) {
            print_r('worker start worker_id:'.$workerId);
            if ($workerId < $this->_config['setting']['worker_num']) {
                swoole_set_process_name('php ' . $this->_config['serverName'] . ' worker');

            } else {
                swoole_set_process_name('php ' . $this->_config['serverName'] . ' task');
            }
        });

        $server->on('WorkerError', function ($server, $workerId, $workerPid, $exitCode) {
            print_r('workerId:'.$workerId.' workerPid:'.$workerPid.' exitCode:'.$exitCode);
        });

        $server->on('WorkerStop', function ($server, $workerId) {
            print_r('workerId:'.$workerId.' stop');
        });
        ...
        $this->createMQProcess('service/test',2);
        $this->_server->start();
    }
    ...
    public function onOpen($server, $frame)
    {
        ...
        print_r($frame);
        $sid = trim(@$frame->get['sid']);
        $uid = trim(@$frame->get['uid']);
        $rid = trim(@$frame->get['rid']);
        $mid = trim(@$frame->get['mid']);
    ...
    public function onMessage($server, $frame)
    {
        ...
        switch ($message['type'])
        {
            ...
            case 'leave':
                ...
                if ($room_id = RoomMap::get($frame->fd))
                {
                    // 删除
                    RoomMember::del($room_id, $frame->fd);
